...
func_kiemloi ({required int MaTin, required String tin, required String ngay, required String mien,}) async{
  gl_loitin = "";
  ...
  try{
    ...
    List<Map> ltbSoHang =await layTable("SELECT CumTu FROM T01_TuKhoa WHERE SoDanhHang='1'");
    ...
    for(String x in lstTin){
      ...
      if(lstMaDai.contains(x) && mien!="B"){
        String sThu = await dLookup("Thu", "T01_MaDai", "MaDai='$x' AND Mien='$mien'");
    ...
    for(String x in lstTin){
      ...
      if((x.length>1 && x[0]=="x" && x[1].isNumeric) || ['xc','dx'].contains(x.substring(0,x.length>=2?2:0)) || x.substring(0,x.length>=3?3:0) == 'dxc'){
        ...
        while (j >= 0){
          ...
          if (await dLookup("GiaTri", "T00_TuyChon", "Ma='kxc'") == 0){
...
Future<bool> Ktra_KieuDanh(String sKieu, String Mien) async{//kiem tra xem so danh biến quy cách số đứng sau có đúng k
  ...
  if (demPhanTu_list(lstKieuDanh, sKieu.substring(0,sKieu.length>=4?4:0)) > 0){// xdau50,'xdui','ddau','ddui'
    ...
    if (tien.isNumeric){
      if (await dLookup("GiaTri","T00_TuyChon","Ma='t'")==1 && sKieu[0]=='t') {ktra=false;}//return
      else {ktra= true;}
    }else {
