...
hamXL(String tin, DateTime ngay,String mien)async {
  ...
  List<Map> ltbMaDai = await layTable("SELECT  * FROM T01_MaDai") ;
  List lstMaDai=[];
  for (var x in ltbMaDai) {
    lstMaDai.add(x["MaDai"]);
  }
  List lstSoHang=await layTable("SELECT CumTu,ThayThe FROM T01_TuKhoa WHERE SoDanhHang=1");
  List<Map> ltbSoHang = [];
  for (var x in ltbSoHang) {
    lstSoHang.add(x['CumTu']);
  }
  List<String> lsbDaiHT = [];
  if(mien!="B"){
    List<MaDaiModel> lstDaiHienTai = await KqxsData().getMaDai(mien, DateFormat("yyyy-MM-dd").format(ngay));
    for(var dai in lstDaiHienTai){
      lsbDaiHT.add(dai.MaDai);
    }
  }
  ...
  for(String x in lstTin){
    ...
    if(x=="dc"  && lsbDaiHT.isNotEmpty){result.add(lsbDaiHT[0]);i+=1;continue;}
    if(x=="dp" && lsbDaiHT.isNotEmpty && lsbDaiHT.length>1){result.add(lsbDaiHT[1]);i+=1;continue;}
  ...
  try{
    ...
    for (String x in result) {
      if(x.length == 1 && k>0 && x.isNumeric && result[k-1].lastChars(1).isNumeric && !result[k-1].lastChars(2).isNumeric){
        result[k-1] += ",$x";
        result[k] = "";
      }else if(x.length==2 && x[0].isNumeric && x[1]=='n' && k>0 && result[k-1].lastChars(1).isNumeric && !result[k-1].lastChars(2).isNumeric){
        result[k-1] += ",$x";
        result[k] = "";
        // result.removeAt(k);
      }
...
List<String> tachDauDui(String str) {
  ...
  List<String> result = [];
  if (arr_sotien.length == 3 && arr_sotien[1] != "" && arr_sotien[2] != "" && arr_sotien[0]=="") {
